...
<Bounded as="footer" class="text-slate-600">
    <div class="container mx-auto flex mt-20 flex-col items-center justify-between gap-6 py-8 sm:flex-row ">
        <div class="name flex flex-col items-center justify-center gap-x-4 gap-y-2 sm:flex-row sm:justify-self-start">
            <a href="/" class="text-xl font-extrabold tracking-tighter text-slate-100 transition-color duration-150 hover:text-yellow-400 focus:text-yellow-400">
                {settings.data.name}
            </a>
        ...
        <nav class="navigation" aria-label="Footer Navigation">
            ...
            <ul class="flex items-center gap-1">
                {#each settings.data.nav_item as {link, label}, index}
                    <li>
                        <PrismicLink field={link} class="block px-3 py-1 text-base font-bold text-slate-100 transition-colors duration-150 hover:text-yellow-400 focus:text-yellow-400">{label}</PrismicLink>
                    </li>
        ...
        <div class="socials inline-flex justify-center sm:justify-end">
            ...
            <PrismicLink field={settings.data.github_link} class="p-2 text-2xl text-slate-300 transform transition-all duration-150 hover:scale-125 hover:text-yellow-400 focus:text-yellow-400 focus:scale-125" 
            aria-label={settings.data.name + " on Github" }>   
                <IconGitHub/>
            </PrismicLink>
            {/if}
            {#if isFilled.link(settings.data.linkedin_link)}
            <PrismicLink field={settings.data.linkedin_link} class="p-2 text-2xl text-slate-300 transform transition-all duration-150 hover:scale-125 hover:text-yellow-400 focus:text-yellow-400 focus:scale-125" 
